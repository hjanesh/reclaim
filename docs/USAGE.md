...
- Press `f` to open a native file/folder picker (Finder on macOS, zenity or kdialog on Linux).
- Press `b` to return to the menu.

Output prompts default to a git-ignored folder inside the repo (`output/photos`, `output/videos`,
etc.). Press Enter to accept the default, or type/pick your own location.

### Background tools and logs

The live dashboard is the source of truth. `photorec` and `ddrescue` run **detached** from the
terminal, with their own output written to a log file in the output folder (e.g.
`output/photos/photorec_run.log`), so their text UI never corrupts the dashboard. If a run seems
stuck, check that log.
